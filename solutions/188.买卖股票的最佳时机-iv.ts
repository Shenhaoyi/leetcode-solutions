...
function maxProfit(k: number, prices: number[]): number {
  ...
  const dp = Array.from({ length: k + 1 /*注意这里是 k+1*/ }, () => new Array<number>(2));
  for (let i = 0; i < length; i++) {
    ...
    for (let j = 0; j < k + 1; j++) {
      ...
      if (i === 0) {
        dp[j][1] = -price; // 开始就持有，说明开始就买了
        dp[j][0] = 0;
        continue;
      }
      const temp = dp[j][1];
      dp[j][1] = Math.max(
        dp[j - 1][0] - price, // 当天买入（即持有），买入时 k 加 1
        dp[j][1], // 之前就持有
      );
      dp[j][0] = Math.max(
        dp[j][0], // 延续 i - 1 的未持有
        temp + price, // i - 1 持有，i 卖出
      );
    }
  }
...
function maxProfit2(k: number, prices: number[]): number {
  ...
  for (let i = 0; i < length; i++) {
    ...
    for (let j = 0; j < k + 1; j++) {
      ...
      if (i === 0) {
        dp[0][j][1] = -price; // 开始就持有，说明开始就买了
        dp[0][j][0] = 0;
        continue;
      }
      dp[i][j][1] = Math.max(
        dp[i - 1][j - 1][0] - price, // 当天买入（即持有），买入时 k 加 1
        dp[i - 1][j][1], // 之前就持有
      );
      dp[i][j][0] = Math.max(
        dp[i - 1][j][0], // 延续 i - 1 的未持有
        dp[i - 1][j][1] + price, // i - 1 持有，i 卖出
      );
    }
  }
